...
def _write_designspace(path: Path) -> None:
    ...
    for master in config.styles["roman"].masters:
...
def _static_font(path: Path, *, width: int, right: int, top: int) -> None:
    ...
    builder.setupHorizontalMetrics({".notdef": (500, 0), "A": (width, 50)})
...
def _build_tiny_two_axis_font(tmp_path: Path) -> TTFont:
    config = load_config(FIXTURE)
    document = DesignSpaceDocument()
    for config_axis in config.axes:
        axis = AxisDescriptor()
...
def test_two_axis_fixture_builds_all_required_variation_tables(tmp_path: Path) -> None:
...
def test_two_axis_binary_build_is_deterministic(tmp_path: Path) -> None:
    variable_font = _build_tiny_two_axis_font(tmp_path)
    first = BytesIO()
    second = BytesIO()
