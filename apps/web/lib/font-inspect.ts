/**
 * Minimal client-side sfnt reader: pulls family/subfamily names, the OS/2
 * weight class, and the italic bit out of a .ttf/.otf ArrayBuffer without any
 * dependency. Mirrors what the Python discover step reads from each donor
 * (usWeightClass + name IDs 16/17 with 1/2 fallback).
 */
...
function assertSupported(view: DataView): void {
  ...
  if (tag === "wOFF" || tag === "wOF2") {
    throw new Error(
      "WOFF/WOFF2 web fonts aren't supported — drop the original .ttf or .otf."
    );
  }
...
export function inspectFont(buffer: ArrayBuffer): FontInfo {
